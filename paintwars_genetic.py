...
from paintwars_config_genetic import *
...
class MyController(Controller):
    ...
    def step(self):
        ...
        if self.team_name == "Team Red":
            translation, rotation = step_red_team(self.id % (len(rob.controllers) / 2), sensors, scores["Team Red"])
        ...
        if (rob.iterations % 100 == 0):
            ...
            for t in tiles:
                if t.owner not in scores:
                    ...
                    scores[t.owner] += 1
            if invertStartingPosition == False:
                print("Scores at iteration #", rob.iterations, ": { Team Red:", scores["Team Red"], ", Team Blue:",
                      scores["Team Blue"], "}")
            else:
